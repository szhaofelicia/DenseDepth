...
# print('original',inputs_original.shape)
# print('inputs',inputs)

# inputs = load_images( glob.glob(args.input) )

# b=inputs.shape[0]
# inputs = cv2.resize(inputs[0], dsize=(640,480)  , interpolation = cv2.INTER_AREA)
...
# outputs=list()
# for i in tqdm(range(int(ALL/BATCH)+1)):
#     inputs_original, inputs = load_images( inputfiles[i*BATCH:min(i*BATCH+BATCH,ALL)])

#     print('\nLoaded ({0}) images of size {1}.'.format(inputs.shape[0], inputs.shape[1:]))
#     print('\nLoaded ({0}) images of original size {1}.'.format(inputs_original.shape[0], inputs_original.shape[1:]))

#     # Compute results
#     outputs.append(predict(model, inputs)) # B* 240 * 320 *1
...
# Display results
# viz = display_images(outputs.copy(), inputs.copy(),is_rescale=False) # default: is_rescale=True
# viz.shape=(480, 1280, 3)
# print(viz)
...
# for i, file in tqdm(enumerate(filenames[:ALL])):
#     frame_depth[filenames[i]]=outputs[i]
...
# pickle.dump(pkl_dict, open("/media/felicia/Data/baseballplayers/pickles/depth_outputs_temp.pkl","wb"))
# print('Saved in pickles.')

# display_results(filenames,outputs, inputs_original, is_colormap=True, is_rescale=True,figsize=(24,8))



"""
python test.py --input  /media/felicia/Data/mlb-youtube/frames_continuous/swing/0ET6W8A0XEKQ0010.jpg 
"""